...
@export var hit_start_time := 0.3

var elapsed_time := 0.0
var hit_area_enabled := false
...
func _on_process(_delta : float) -> void:
	elapsed_time += _delta
	if not hit_area_enabled and elapsed_time >= hit_start_time:
		_set_hit_area_enabled(true)
		hit_area_enabled = true
...
func _on_enter() -> void:
	...
	var animation_name := "chopping_front"
	match player.player_direction:
		Vector2.LEFT:
			animation_name = "chopping_left"
		Vector2.RIGHT:
			animation_name = "chopping_right"
		Vector2.UP:
			animation_name = "chopping_back"
		Vector2.DOWN:
			animation_name = "chopping_front"
