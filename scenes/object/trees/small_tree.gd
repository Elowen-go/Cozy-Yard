...
func _ready() -> void:
	if material is ShaderMaterial:
		...
		tree_material = material as ShaderMaterial
		tree_material.set_shader_parameter("shake_intensity", 0.0)
...
func _play_hit_shake() -> void:
	if not tree_material:
		return

	if shake_tween:
		shake_tween.kill()

	tree_material.set_shader_parameter("shake_intensity", hit_shake_intensity)
...
func _on_max_damage_reached() -> void:
	...
	is_destroying = true
	call_deferred("_drop_log_and_destroy")


func _drop_log_and_destroy() -> void:
	if not is_inside_tree():
		return

	if log_scene and get_parent():
